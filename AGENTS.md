...
All Beads Dolt pushes disable the optional visible info branch:

```bash
DOLT_REMOTE_INFO_BRANCH='' bd dolt push
```

Do not run a bare `bd dolt push`; it recreates `__dolt_remote_info__`.
...
- **Team-maintainer**: Only when the repository explicitly opts in, agents may close
  beads, run quality gates, commit, and push as part of session close. A current
  "do not commit" or "do not push" instruction still wins.

## Session Completion

This protocol applies when ending a Beads implementation workflow. It is subordinate
to explicit user, repository, and orchestrator instructions.
...
`~/.agents/skills/beads/SKILL.md` (global install) for Beads workflow guidance, then
use the `bd` CLI for issue operations.
...
- Keep persistent project memory in Beads via `bd remember`; do not create ad hoc
  memory files.
